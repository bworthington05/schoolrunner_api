...
# Requires endpoint URL, endpoint name, list of fields, username, and password
...
def get_data(url, endpoint_name, fields, user, pw):
    ...
    # Loop through each item in the bucket and loop through each item in fields list
    # Parse the JSON data and save the fields we want
    for i in bucket:
        for f in fields:
            temp_data.append(i.get(f))
